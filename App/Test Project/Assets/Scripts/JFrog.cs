...
using UnityEngine;
/**
 * script encargado del total de las acciones del jugador de nombre Frog
...
public class JFrog : MonoBehaviour
{
    ...
    void Update()
    {
        if (jugador.transform.position.y < (float) -8.4197)
        {
            if (Challenge == "C")
            {
        ...
        {
            puntajefrog += 50;
            (gameManager.PuntajeFrog).text = puntajefrog.ToString();
        ...
        {
            if (gameManager.Jugador4.text == "")
            {
                Destroy(GameObject.FindWithTag("Personaje"));
            }
            if (Input.GetKeyDown(KeyCode.UpArrow))
            {
                //GameObject a = Instantiate(s_salto);
                ...
                if(!salta)
                {
                    if (Saltos >= 1)
                    {
                        ...
                        rigidbody2D.AddForce(new Vector2(0, FuezadeSalto));
                        Saltos--;
                    }
                    else
                    {
            ...
            if (Input.GetKeyDown(KeyCode.Space))
            {
                if (force_push)
                {
                    if (M_derecha)
                    {
                        rigidbody2D.AddForce(new Vector2(FuerzadeLado+150, 0));
                    }
                    if (M_izquierda)
                    {
                        rigidbody2D.AddForce(new Vector2(-(FuerzadeLado +150), 0));

...
using UnityEngine;
/**
 * script encargado del total de las acciones del jugador de nombre Blue
        ...
        if (gameManager.start)
        {
            ...
            if (Input.GetKeyDown(KeyCode.Y))
            {
                if (force_push)
                {
                    if (M_derecha)
                    {
                        _rigidbody2D.AddForce(new Vector2(FuerzadeLado+150, 0));
                    }
                    if (M_izquierda)
                    {
                        _rigidbody2D.AddForce(new Vector2(-(FuerzadeLado+150), 0));
